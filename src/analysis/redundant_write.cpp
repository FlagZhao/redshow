...
void RedundantWrite::unit_access(i32 kernel_id, const ThreadId &thread_id,
  ...
  // use value as size of access here for different types. aligned to 4 bytes.
  if(value > 4){
  ...
  if(flags & GPU_PATCH_SHARED){
    ...
    if(last_access_record_shared == _trace->shared_memory_access_history[thread_id.flat_block_id][memory].end()){
      ...
      if(last_access_record_shared->second._thread_id == thread_id){
        if(last_access_record_shared->second._access_type == flags){
          //Pure redundancy 
          if(flags & GPU_PATCH_WRITE){
            // PRINT("Redundant WS Detected: Memory:%p,\t Blockid:%d,\t Threadid:%d,\t PC:%lx, previous PC is:%lx\n",addr,thread_id.flat_block_id,thread_id.flat_thread_id,pc,last_access_record_shared->second._pc);        
          }else if(flags & GPU_PATCH_READ){
            // PRINT("Redundant RS Detected: Memory:%p,\t Blockid:%d,\t Threadid:%d,\t PC:%lx\n",addr,thread_id.flat_block_id,thread_id.flat_thread_id,pc);
          }
          stat.redundant_accesses+=1;
        }else{
          //Read After Write
          if((last_access_record_shared->second._access_type & GPU_PATCH_WRITE) && (flags & GPU_PATCH_READ)){
            stat.redundant_accesses+=1;
            // PRINT("Redundant WRS Detected: Memory:%p,\t Blockid:%d,\t Threadid:%d,\t PC:%lx\n",addr,thread_id.flat_block_id,thread_id.flat_thread_id,pc);
          }
        }
      }
      _trace->shared_memory_access_history[thread_id.flat_block_id][memory].insert_or_assign(addr, AccessRecord_t(thread_id, pc, flags));
  ...
  if(last_access_record == _trace->access_history[memory].end()){
    ...
    if(last_access_record->second._thread_id == thread_id){
      ...
      if(last_access_record->second._access_type == flags){
        ...
        //Read After Write
        if((last_access_record->second._access_type & GPU_PATCH_WRITE) && (flags & GPU_PATCH_READ)){
          stat.redundant_accesses+=1;
...
void RedundantWrite::flush_thread(u32 cpu_thread, const std::string &output_dir,
  ...
  for (auto &kernel_trace_it : *thread_kernel_trace) {
    ...
    ofs<<"kernel:"<<kernel_trace_it.first<<endl; //print kernel id
    // cout<<"kernel:"<<kernel_trace_it.first<<endl; //print kernel id
    int fully_redundant = 0;
    int partially_redundant = 0;
    int slightly_redundant = 0;
    int pc_count = 0;
    for(const auto& [pc,stats] : std::dynamic_pointer_cast<RedundantWriteTrace>(kernel_trace_it.second)->pc_stats){
      ...
      ofs  << "pc: " << std::hex << pc << std::dec 
            << " \ttype: ";
      if(stats.access_type & GPU_PATCH_WRITE){
        ofs << "W";
      }
      if(stats.access_type & GPU_PATCH_READ){
        ofs << "R";
      }
      if(stats.access_type & GPU_PATCH_SHARED){
        ofs << "S";
      }
      if(redundancy_rate == 1){
        fully_redundant +=1;
      }else if(redundancy_rate >= 0.5){
        partially_redundant +=1;
      }else if(redundancy_rate > 0){
        slightly_redundant +=1;
      }
      pc_count+=1;
      // if(stats.access_type & GPU_PATCH_ATOMIC){
      //   ofs << "A";
      // }
      ofs<< " \ttotal_accesses: " << stats.total_accesses 
    ...
    ofs <<"pc_count:"<< pc_count << "\tfully_redundant: " << fully_redundant << " \tpartially_redundant: " << partially_redundant << " \tslightly_redundant: " << slightly_redundant << endl << std::flush;
